...
main: 
	...
	MOV R4, #0 //Num calls = 0
	MOV R1, #5 //key = 5
	MOV R2, #0 //start index = 0
	MOV R3, #8 //end index = 8
	LDR R0, =numbers
	LDR R9, =LEDR_BASE
	LDR R10, =SW_BASE
	BL b_search		
...
b_search:
	...
	STR R4, [sp, #32] /*num calls*/
	
	LDR R3, [sp, #12] /*R3 = end*/
	LDR R2, [sp, #8] /*R2 = start*/
	SUB R3, R3, R2 // int middleIndex = endIndex - startIndex; 
	LSR R3, R3, #1 // int middleIndex = (endIndex - startIndex)/2 : shifting to the right by 1 divides by 2
...
L3: LDR R3, [sp, #16] /*R3 = middle index*/
	SUB R3, R3, #1 /*R2 = middle index -1 (start)*/
	...
	LDR R2, [sp, #8] /*start index*/
	LDR R3, [sp, #16] /*middle*/
	LDR R4, [sp, #32] /*num calls*/
	LDR lr, [sp, #20] /*link reg*/
	MOV R3, R0 /*R3 = binary_search(updated values)*/
	STR R3, [R12] /*R3 is return value of b_search: key index = binary_search(...)*/
	B L5

L4: LDR R3, [sp, #16] /*R3 = middle index*/
	ADD R3, R3, #1 /*R3 = middle index + 1*/
	//STR R3, [sp, #8] /*start = middle index + 1*/
	LDR R0, [sp, #0] /*the array*/
	LDR R1, [sp, #4] /*key*/
	LDR R2, [sp, #8] /*start index*/
	LDR R4, [sp, #32] /*num calls*/
